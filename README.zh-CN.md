...
│ ❯ 跑一遍测试，把失败的用例总结给我                             │
╰────────────────────────────────────────────────────────────────╯
  / 命令 · ↑ 历史 · \ + enter 换行   ⠹ working 12s · deepseek-v4-flash high · ctx 12%
```
...
- **工具调用显示它做了什么**，而不只是一个名字。dsh 会把每个事件渲染成有类型的卡片
  —— terminal、read、search、diff、web —— 窗格把每一种都画出来：命令失败先显示退出码，
  读文件显示行数和路径，搜索显示命中数。结果默认最多 5 行，**ctrl+r** 把最近被截断的
  那条完整打印出来。
- **状态行**显示状态、已用秒数、模型 + 思考强度，以及会话占用了多少上下文
  （`ctx 12%` —— 超过 70% 变黄，超过 90% 变红），由 host 的 projection 事件流实时驱动。
